...
# apply `bootPower` multiple times, to assess PERMANOVA power with a different number of subjects per group
# ω2 smaller than the effects observed in studies of antibiotic exposure (0.1) analyzed with weighted Jaccard distances
# likely affords adequate statistical power for the primary outcome measure.
...
for (s in c(5,6,7,8)) {
  ...
  p <- lapply(p, FUN = function(x) {
    simStudy(group_size_vector, otu_number, sequence_depth, 
             rare_depth, x)
