...
## ----load-otu----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
otu<-readRDS("RDS/full-0.01-af-bracken-max-highoral-0.01-otu-env-filtered.rds")
samples<-data.frame(sample_data(otu)) %>% 
  filter(Bear.oral.health%in%c("healthy","caries")) %>% 
  pull(SampleID)
df<-otu %>% 
...
ggsave(plot=p1,file="means.png",dpi=300)

## ---- hashSD, warning=FALSE--------------------------------------------------------------------------------------------------------------------------------------------------------------------
weight_hsd<-hashSD(rare_depth=subsample,otu_number_range=10^runif(n = 100,min = 1,max = 5),sim_number=row(samples), sequence_depth=1000)
...
ggsave(plot=p2,file="otu-sd.png",dpi=300)


## ---- simPower---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
sp<-simPower(group_size_vector=c(row(samples),row(samples)), otu_number=otunum, rare_depth=subsample,sequence_depth=1000,effect_range=seq(0,0.3,length.out=100))
...
## ---- bootPower--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bp80<-bootPower(wj, boot_number=10, subject_group_vector=c(row(samples),row(samples)),alpha=0.05)
...
p3<-ggplot2::qplot(data=bp80_model,x=log_power,y=log_omega2) + geom_smooth(method="lm") + ggtitle(paste0("log10(Omega2) to log10(Power) with",nrow(samples), "Subjects/Group")) + xlab("log10(Power)") + ylab("log10(Omega2)")
ggsave(plot=p3,file="omega-power-80.png",dpi=300)